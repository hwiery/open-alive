...
export function getPaths(rootOverride?: string): Paths {
  const root = rootOverride ?? process.env.OPEN_ALIVE_PROMPT_HOME ?? join(homedir(), '.open-alive', 'prompt');
  return {
    root,
    ...
    claudeSettings:
      process.env.OPEN_ALIVE_PROMPT_CLAUDE_SETTINGS ?? join(homedir(), '.claude', 'settings.json'),
  };
}
