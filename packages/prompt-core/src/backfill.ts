 ...
 * the open-alive prompt DB, and how many would be new imports.
 *
 * Also exposes `importClaudeHistory` to actually write those prompts into
 * the local DB (used by `open-alive-prompt backfill --execute`). Import runs
...
/** Signature of `runRules` from @open-alive/prompt-rules. Kept loose here to avoid
