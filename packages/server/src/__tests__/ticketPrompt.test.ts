...
describe('buildOrchestratorPrompt', () => {
  ...
  it('embeds the delegate command and the default model it was given', () => {
    const out = buildOrchestratorPrompt('goal', '', '/bin/oa-delegate', 'glm-5.2');
    expect(out).toContain('/bin/oa-delegate --model <모델> "<하위 작업 프롬프트>"');
  ...
  it('keeps the guide prefix and the HEADLINE suffix', () => {
    const out = buildOrchestratorPrompt('goal', 'Learned: prefer X', '/bin/oa-delegate', 'kimi-k3');
    expect(out.startsWith('Learned: prefer X\n\n---\n')).toBe(true);
...
describe('remote commit instruction', () => {
  ...
  it('reaches the orchestrator prompt too', () => {
    const p = buildOrchestratorPrompt('goal', '', '/bin/oa-delegate', 'm', { askToCommit: true });
    expect(p).toContain('git commit');
    expect(p.indexOf('git commit')).toBeLessThan(p.indexOf('HEADLINE:'));
  });
});
...
describe('DECISION contract', () => {
  ...
  it('reaches the orchestrator prompt too', () => {
    expect(buildOrchestratorPrompt('goal', '', '/bin/oa-delegate', 'm')).toContain('한 번에 하나만 묻습니다');
  });
});
