 ...
 * D-048 (open-alive-prompt absorbed into open-alive) the dashboard role
 * is gone; the React Prompt tab inside open-alive's UI replaces it.
 *
 *   macOS  → ~/Library/LaunchAgents/com.openalive.prompt.<role>.plist (launchd)
 *   Linux  → ~/.config/systemd/user/open-alive-prompt-<role>.service (systemd --user)
 *
 * Sub-commands:
 ...
 *
 * Design note: this is OPT-IN. `open-alive-prompt install` does NOT auto-enable
...
function unitFileName(p: Platform, role: Role): string {
  return p === 'darwin' ? `com.openalive.prompt.${role}.plist` : `open-alive-prompt-${role}.service`;
}
...
<dict>
  <key>Label</key>
  <string>com.openalive.prompt.${role}</string>
...
export function buildSystemdUnit(
  ...
  const logFile = `${root}/autostart-${role}.log`;
  return `[Unit]
Description=open-alive-prompt ${role} daemon
...
function buildContent(p: Platform, role: Role): string {
  ...
  const root = `${homedir()}/.open-alive/prompt`;
  return p === 'darwin'
...
function loadUnit(p: Platform, file: string, role: Role): void {
  if (p === 'darwin') {
    ...
    execFileSync('systemctl', ['--user', 'enable', '--now', `open-alive-prompt-${role}.service`], {
      stdio: 'inherit',
    });
  }
}
...
function unloadUnit(p: Platform, file: string, role: Role): void {
  if (p === 'darwin') {
    ...
    try {
      execFileSync('systemctl', ['--user', 'disable', '--now', `open-alive-prompt-${role}.service`], {
        stdio: 'inherit',
      });
    } catch {
      // already disabled
...
function isActive(p: Platform, role: Role): boolean {
  if (p === 'darwin') {
    ...
    const line = out.split('\n').find((l) => l.includes(`com.openalive.prompt.${role}`));
    if (!line) return false;
    const pid = line.split(/\s+/)[0];
    return /^\d+$/.test(pid ?? '') && pid !== '0';
  }
  const out = tryRun('systemctl', ['--user', 'is-active', `open-alive-prompt-${role}.service`]).trim();
  return out === 'active';
}
...
export function autostartEnableCmd(): void {
  ...
  console.log(pc.dim('\nVerify: open-alive-prompt autostart status'));
}
