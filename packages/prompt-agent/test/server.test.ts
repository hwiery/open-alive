...
beforeEach(() => {
  ...
  process.env.OPEN_ALIVE_PROMPT_HOME = tmp;
});

afterEach(() => {
  rmSync(tmp, { recursive: true, force: true });
  delete process.env.OPEN_ALIVE_PROMPT_HOME;
});
...
describe('agent server', () => {
  ...
  it('coach_mode injects additionalContext for bad prompts', async () => {
    ...
    expect(body.hookSpecificOutput.additionalContext).toContain('open-alive prompt coaching hint');
  ...
  it('post-tool-use upserts unknown session (FK-safe, regression for #11)', async () => {
    const { openDb } = await import('@open-alive/prompt-core');
    const app = buildAgentServer({ rootOverride: tmp });
  ...
  it('/v1/ingest/web stores + scores + returns tier', async () => {
    ...
    const res = await app.inject({
      method: 'POST',
      url: '/v1/ingest/web',
      headers: { 'x-open-alive-prompt-ext': '1' },
      payload: {
        source: 'chatgpt',
  ...
  it('/v1/ingest/web rejects requests missing the X-Open-Alive-Prompt-Ext header', async () => {
    const app = buildAgentServer({ rootOverride: tmp });
    const res = await app.inject({
      method: 'POST',
      url: '/v1/ingest/web',
  ...
  it('/v1/ingest/web OPTIONS preflight responds 204 with CORS + PNA headers', async () => {
    ...
    const res = await app.inject({
      ...
      headers: {
        ...
        'access-control-request-headers': 'content-type, x-open-alive-prompt-ext',
      },
    });
    expect(res.statusCode).toBe(204);
    expect(res.headers['access-control-allow-origin']).toBe('chrome-extension://abcdef');
    expect(res.headers['access-control-allow-methods']).toContain('POST');
    expect(res.headers['access-control-allow-headers']).toContain('x-open-alive-prompt-ext');
