import { SessionStore, parseTranscriptTokens } from '@open-alive/core';
import type { HookEventPayload, Ticket, TicketLocation } from '@open-alive/core';
import { isRemoteLocation, editedPathFrom } from '@open-alive/core';
import { createPromptSubsystem, type PromptSubsystem } from '@open-alive/prompt-agent';
...
// The server usually runs detached (`open-alive start`), so it never sees the
// interactive shell's exports. Read ~/.open-alive/.env first — everything
// below (LITELLM_KEY, OA_DELEGATE_MODEL, the port) reads process.env directly.
const envFromFile = loadServerEnv();
if (envFromFile.length > 0) {
  console.log(`[server] loaded ${envFromFile.join(', ')} from ${SERVER_ENV_FILE}`);
}

const PORT = parseInt(process.env.OPEN_ALIVE_PORT ?? '3141', 10);
...
// Absorbed open-alive-prompt subsystem. Owns its own SQLite handle and a
...
const canonicalPipeline = createCanonicalPipeline({
  dbPath: process.env.OPEN_ALIVE_EVENT_DB ?? join(ALIVE_DIR, 'alive.db'),
  locationId: 'local',
...
// Disabled if efficio source isn't found or OPEN_ALIVE_AUTO_COLLECT=0.
const efficioCollector = createEfficioCollector({
  efficioRoot: process.env.OPEN_ALIVE_AUTO_COLLECT === '0' ? null : resolveEfficioRoot(),
...
const runStore = createRunStore({ file: join(homedir(), '.open-alive', 'runs.json') });
await runStore.load();
...
// Write the `oa-delegate` sub-agent tool and capture its absolute path, embedded
...
// oa-delegate` resolves — the verifier must know the delegation tool is real.
...
if (!delegateCmd) {
  console.log('[server] oa-delegate not available in this install — tickets run without orchestration');
}
...
const localAllowedRoots = process.env.OPEN_ALIVE_TICKET_ROOTS?.split(':').filter(Boolean);
...
// Auto-commit after a passing verdict. `OPEN_ALIVE_AUTO_COMMIT=0` disables it
// server-wide; a single ticket opts out with `autoCommit: false` at creation.
const autoCommitEnabled = process.env.OPEN_ALIVE_AUTO_COMMIT !== '0';
...
// ticket and per directory tree (OPEN_ALIVE_PANEL_EXCLUDE). An excluded ticket
...
const panelFor = (ticket: import('@open-alive/core').Ticket) =>
 ...
 */
const askRemoteToCommit = (ticket: import('@open-alive/core').Ticket) =>
...
const ticketRunner = createTicketRunner({
  ...
  spawnMain: (ticket, opts) => {
    ...
    // Only for local execution (the oa-delegate CLI lives on the server host, so
    ...
    return executorFor(ticket.location).spawn({
      ...
      // verifier deliberately omits OA_TICKET_ID so its re-delegations aren't logged.
      ...(orchestrated && delegateBinDir
        ? { pathPrepend: delegateBinDir, extraEnv: { OA_TICKET_ID: ticket.id } }
        : {}),
    });
  },
  ...
  ...(Number(process.env.OPEN_ALIVE_TICKET_CONCURRENCY) > 0
    ? { concurrency: Number(process.env.OPEN_ALIVE_TICKET_CONCURRENCY) }
    : {}),
});
if (!process.env.OPEN_ALIVE_TICKET_ROOTS) {
  ...
  console.warn(
    '[tickets] OPEN_ALIVE_TICKET_ROOTS is unset — autonomous agents may run in any cwd. ' +
...
function validateRemoteTicketCreate(input: { cwd: string; location?: { kind: string; ssh?: { host: string } } }): string | null {
  if (input.location?.kind === 'ssh') {
    ...
    if (!host || !remoteAccess.sshHosts.includes(host)) {
      return '이 호스트로는 원격 티켓을 만들 수 없습니다. OPEN_ALIVE_REMOTE_SSH_HOSTS 에 추가하세요.';
    }
    return null;
  }
...
const httpServer = createHttpServer({
  ...
  gatewaySettings: createGatewaySettings({
    ...
    modelsFile: process.env.OA_DELEGATE_MODELS_FILE?.trim() && process.env.OA_DELEGATE_MODELS_FILE.trim() !== 'builtin'
      ? process.env.OA_DELEGATE_MODELS_FILE.trim()
      : MODELS_FILE,
    env: process.env,
    onApply: () => {
...
httpServer.listen(PORT, HOST, () => {
  ...
  if (remoteAccess.enabled) {
    ...
    if (remoteAccess.terminalLevel === 'shell') {
      console.warn(
        '[server] OPEN_ALIVE_REMOTE_TERMINAL=shell — a device token can start a shell on this ' +
          'machine. Revoke it (open-alive token revoke <label>) and restart if a device is lost.',
      );
    }
    if (remoteAccess.trustLoopback) {
      console.warn(
        '[server] OPEN_ALIVE_TRUST_LOOPBACK=1 — anything reaching this server through a local ' +
