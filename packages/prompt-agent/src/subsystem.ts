/**
 * Prompt subsystem — the absorbed open-alive-prompt capture pipeline as an
 * in-process library, not a standalone daemon.
 *
 * Returned shape:
 *   - ingest(payload)  : unified hook dispatcher called from open-alive's
 ...
 *                         open-alive's shared http.Server — never opens
 *                         its own port.
 *   - close()          : releases the SQLite handle on shutdown.
 *
 * The coaching-hint response (D-021) is *not* preserved on the unified
 * path: open-alive's bash hook discards the POST response, so there is
...
import {
  ...
  upsertSubagent,
  upsertQualityScore,
} from '@open-alive/prompt-core';
import { composeFinalScore, computeRuleScore } from '@open-alive/prompt-core';
import { runRules } from '@open-alive/prompt-rules';
...
export function createPromptSubsystem(deps: PromptSubsystemDeps = {}): PromptSubsystem {
  ...
  fastify.addHook('onSend', async (req, reply, payload) => {
    ...
    reply.header('access-control-allow-headers', 'content-type, x-open-alive-prompt-ext');
  ...
  fastify.post('/v1/ingest/web', async (req, reply) => {
    const t0 = Date.now();
    const extHeader = req.headers['x-open-alive-prompt-ext'];
    if (extHeader !== '1') {
      reply.code(403);
      return { ok: false, error: 'missing X-Open-Alive-Prompt-Ext header' };
    }
    try {
