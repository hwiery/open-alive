...
describe('parseEnvFile', () => {
  it('reads KEY=VALUE pairs and skips comments and blanks', () => {
    const out = parseEnvFile('# comment\n\nLITELLM_KEY=sk-abc\nOA_DELEGATE_MODEL=glm-5.2\n');
    expect(out).toEqual({ LITELLM_KEY: 'sk-abc', OA_DELEGATE_MODEL: 'glm-5.2' });
  });

  it('tolerates `export ` and strips one layer of matching quotes', () => {
    const out = parseEnvFile('export LITELLM_BASE_URL="http://localhost:4000"\nNOTE=\'two words\'\n');
    expect(out).toEqual({ LITELLM_BASE_URL: 'http://localhost:4000', NOTE: 'two words' });
  });
...
describe('loadServerEnv', () => {
  ...
  // An explicit `LITELLM_KEY=… open-alive start` must beat the file, so a
