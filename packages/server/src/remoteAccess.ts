 ...
 * for by name (`OPEN_ALIVE_TRUST_LOOPBACK=1`).
 */
 ...
 * floor for the 32-byte base64url token `open-alive token` generates.
 */
export const MIN_TOKEN_CHARS = 24;

/** WebSocket clients cannot set headers, so the token rides the subprotocol. */
export const WS_TOKEN_PROTOCOL_PREFIX = 'open-alive.token.';
...
export interface RemoteAccessConfig {
   ...
   * answer is filesystem access: this value sits 0600 in ~/.open-alive/.env,
...
/**
 * Read tokens from `OPEN_ALIVE_TOKEN` (one) and `OPEN_ALIVE_TOKENS`
...
export function parseTokens(env: NodeJS.ProcessEnv): RemoteToken[] {
  ...
  const single = env.OPEN_ALIVE_TOKEN?.trim();
  if (single) out.push({ label: 'default', value: single });
  const list = env.OPEN_ALIVE_TOKENS?.trim();
  if (list) {
    let n = 0;
...
export interface AuthorizeInput {
   ...
   * header, so `open-alive start` hands it the token in the URL once and the
...
export function loadRemoteAccessConfig(env: NodeJS.ProcessEnv): ConfigResult {
  const enabled = truthy(env.OPEN_ALIVE_REMOTE);
  const terminalLevel = parseRemoteTerminalLevel(env.OPEN_ALIVE_REMOTE_TERMINAL);
  const explicitHost = env.OPEN_ALIVE_HOST?.trim();
  ...
  if (!enabled && !hostIsLoopback) {
    errors.push(
      `OPEN_ALIVE_HOST=${host} exposes the server beyond this machine. Set OPEN_ALIVE_REMOTE=1 ` +
        'to opt in (which also requires OPEN_ALIVE_TOKEN and OPEN_ALIVE_TICKET_ROOTS).',
    );
  }

  const tokens = parseTokens(env);
  const ticketRoots = (env.OPEN_ALIVE_TICKET_ROOTS ?? '').split(':').map((s) => s.trim()).filter(Boolean);
  const sshHosts = (env.OPEN_ALIVE_REMOTE_SSH_HOSTS ?? '').split(',').map((s) => s.trim()).filter(Boolean);

  if (enabled) {
    if (tokens.length === 0) {
      errors.push('OPEN_ALIVE_REMOTE=1 requires OPEN_ALIVE_TOKEN (or OPEN_ALIVE_TOKENS). Run: open-alive token new');
    }
    for (const token of tokens) {
      if (token.value.length < MIN_TOKEN_CHARS) {
        errors.push(`Token "${token.label}" is shorter than ${MIN_TOKEN_CHARS} characters. Run: open-alive token new`);
      }
    }
    if (ticketRoots.length === 0) {
      errors.push(
        'OPEN_ALIVE_REMOTE=1 requires OPEN_ALIVE_TICKET_ROOTS — a remote caller can start an autonomous agent, ' +
  ...
  return {
    ...
    config: {
      enabled,
      host,
      trustLoopback: truthy(env.OPEN_ALIVE_TRUST_LOOPBACK),
      tokens,
      ticketRoots,
      sshHosts,
      terminalLevel,
      ...(env.OPEN_ALIVE_LOCAL_TOKEN?.trim() ? { localToken: env.OPEN_ALIVE_LOCAL_TOKEN.trim() } : {}),
    },
  };
}
