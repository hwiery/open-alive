import { loadConfig, saveConfig, setConfigValue } from '@open-alive/prompt-core';
...
export async function coachCmd(state: string): Promise<void> {
  if (state !== 'on' && state !== 'off') {
    console.log(pc.red('usage: open-alive-prompt coach <on|off>'));
    return;
  }
  const cfg = loadConfig();
  ...
  console.log(pc.dim('(restart the agent for the change to take effect: `open-alive-prompt restart`)'));
}
