/**
 * Server-side env file loading (`~/.open-alive/.env`).
 *
 * The server is started as a detached daemon (`open-alive start`, launchd),
 ...
 * an explicit `LITELLM_KEY=… open-alive start` still overrides the file.
 *
...
export const SERVER_ENV_FILE = join(homedir(), '.open-alive', '.env');
...
/**
 * Merge `~/.open-alive/.env` into the process env without overwriting values
