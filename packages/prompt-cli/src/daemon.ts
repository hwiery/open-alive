...
/**
 * Absorbed into open-alive (D-048): the `dashboard` role is gone — the
 * React-based Prompt tab in open-alive replaces the eta+Alpine dashboard.
...
export function resolveEntry(role: Role): string {
  ...
  try {
    return require.resolve(`@open-alive-prompt/${role}`);
  } catch {
