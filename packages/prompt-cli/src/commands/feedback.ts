import { type OutcomeRating, openDb, recordOutcome } from '@open-alive/prompt-core';
import pc from 'picocolors';

/**
 * `open-alive-prompt feedback <usage_id> <up|down> [--note "..."]`
 *
...
export async function feedbackCmd(
  ...
  console.log(
    pc.dim(
      `  (Run \`open-alive-prompt reprocess --session <id>\` after a session ends to refresh usage_score.)`
    )
  );
