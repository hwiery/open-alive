/**
 * `open-alive-prompt analyze <id>` — the CLI entry point for deep analysis.
 *
...
export async function analyzeCmd(id: string | undefined, opts: AnalyzeCmdOptions): Promise<void> {
  ...
  if (!config.llm.enabled) {
    ...
    console.log('    open-alive-prompt config set llm.enabled true');
  ...
  if (config.analysis.deep_consent !== 'granted') {
    ...
    console.log('');
    console.log('  To grant consent:   ' + pc.bold('open-alive-prompt analyze --grant-consent'));
    console.log('  To decline:         ' + pc.bold('open-alive-prompt analyze --revoke-consent'));
    process.exit(1);
    return;
  }
