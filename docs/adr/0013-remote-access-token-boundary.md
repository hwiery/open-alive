...
1. 기본 바인딩은 `127.0.0.1`. 다른 인터페이스는 `OPEN_ALIVE_REMOTE=1` 로만 열리고, 토큰과 `OPEN_ALIVE_TICKET_ROOTS` 가 없으면 **부팅을 거부**한다(경고가 아니라).
2. 원격 모드에서 **소스 주소는 증거가 아니다.** loopback 을 포함해 모든 요청이 bearer 토큰을 제시한다. 프록시가 앞에 없는 설치는 `OPEN_ALIVE_TRUST_LOOPBACK=1` 로 예전 동작을 되살릴 수 있으나 기본은 꺼짐.
...
6. 로컬 구성요소(훅 스크립트·CLI·서버가 서빙하는 대시보드)는 `~/.open-alive/.env`(0600)에 생성되는 **전권 로컬 토큰**을 쓴다. 로컬임을 증명하는 것은 소스 주소가 아니라 그 파일을 읽을 수 있다는 사실이다.
...
- 구현: `packages/server/src/remoteAccess.ts`, `wsAuth.ts`, `localToken.ts`, `httpRouter.ts` 게이트, `packages/ui/src/lib/auth.ts`, `open-alive token`, 훅 스크립트 인증.
