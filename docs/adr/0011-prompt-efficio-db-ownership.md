...
## 맥락
저장소에 SQLite DB가 3개 있고 소유자와 접근 권한이 문서화되지 않았다. canonical 이벤트 로그가 추가되면서 "어느 DB가 무엇의 출처인가"를 고정하지 않으면 중복 기록과 상충하는 집계가 발생한다.
...
| DB | 소유자 | Alive 서버의 접근 |
|---|---|---|
| `~/.open-alive/prompt/prompts.db` | `prompt-core` | prompt 서브시스템만 읽기/쓰기. canonical 계층은 접근 금지 |
| `~/.efficio/efficio.db` | **외부 efficio (Python)** | **read-only.** Alive는 통계를 계산하지 않고 사전 채점된 행만 읽는다 |
| canonical event log (`storage`) | `@open-alive/storage` | canonical 계층만 읽기/쓰기. prompt/efficio는 접근 금지 |
