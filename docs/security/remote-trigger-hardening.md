...
목표: 앱에서 로컬 open-alive를 **같은 네트워크 밖에서** 제어(티켓 생성/트리거)하되, 오픈소스 배포물로서 안전한 기본값과 인증을 갖춘다.
...
| `POST /v1/ingest/web` + `X-Open-Alive-Prompt-Ext: 1` | **200. 프롬프트 저장소 원격 쓰기** | 높음 |
...
|---|---|---|---|---|---|
| **Bearer 토큰** (`OPEN_ALIVE_TOKEN`) | ✗(별도 필요) | O | 단순·명확 | 낮음 | **채택**(1차) |
...
- 저장: `~/.open-alive/.env`(이미 `serverEnv.ts`가 읽음, 신규 로더 불필요). 파일 권한 **0600** 강제·경고.
...
- **기기별 토큰**: 단일 공유 비밀은 분실 기기만 폐기할 수 없다. 최소한 `OPEN_ALIVE_TOKENS`(콤마 구분, 라벨 포함) 형태로 복수 토큰을 받고 라벨 단위로 지울 수 있게 한다.
...
1. **바인딩 기본값 복구**
   - `OPEN_ALIVE_HOST` 도입, 기본 `127.0.0.1`. `listen(PORT, HOST)`.
   - 회귀 수정이므로 원격 기능과 무관하게 **선행 단독 커밋** 권장.

2. **노출 opt-in + 토큰 강제**
   - `OPEN_ALIVE_REMOTE=1`일 때만 외부 바인딩 허용.
   - 이때 `OPEN_ALIVE_TOKEN` 미설정 → **부팅 거부**(경고 아님).
   - `OPEN_ALIVE_TICKET_ROOTS` 미설정 → 원격 모드 부팅 거부(현재는 경고만). 단 이 allowlist는 **로컬 티켓에만** 적용된다(`index.ts:501`) — 원격 모드에서 `location.kind === 'ssh'` 티켓은 별도 정책(호스트 allowlist)이 필요하다.
...
3. **게이트 반전 + 화이트리스트 (rev2 수정)**
   - 판정식(수정): `remoteEnabled ? (validToken(req) && isRemoteAllowed(method, path)) : isLoopbackRequest(req)`.
     ...
     - 로컬 UI도 같은 규칙을 타야 하므로, 서버가 서빙하는 UI에 토큰을 주입하거나(부팅 시 생성된 값) 로컬 전용 예외를 `OPEN_ALIVE_TRUST_LOOPBACK=1`로 **명시 opt-in** 시킨다.
...
### 구현 게이트 — 서버
- [x] `OPEN_ALIVE_HOST` 기본 `127.0.0.1` 바인딩 — `index.ts` `listen(PORT, HOST)`
- [x] `OPEN_ALIVE_REMOTE` opt-in, 미설정 시 loopback
- [x] 원격 모드에서 토큰 부재 시 부팅 거부
- [x] 원격 모드에서 `OPEN_ALIVE_TICKET_ROOTS` 부재 시 부팅 거부 + ssh 호스트 allowlist(`OPEN_ALIVE_REMOTE_SSH_HOSTS`)
...
### 구현 게이트 — CLI/앱/문서
- [x] CLI: `open-alive start --remote/--host`, 부팅 거부 시 로그 노출
- [x] CLI: `open-alive token new|list|revoke`, `status` 가 로컬 토큰으로 인증
...
- [ ] `open-alive doctor` 에 원격 모드 점검 항목 — **미착수**
...
curl -s -o /dev/null -w '%{http_code}\n' -X POST -H 'Content-Type: application/json' \
  -H 'X-Open-Alive-Prompt-Ext: 1' -d '{}' "http://$IP:3141/v1/ingest/web"   # 200 → 헤더만으로 원격 쓰기
...
뒤의 셋은 §4.3에서 원격 차단 대상이다 → **원격에서 티켓을 만들려면 대체 엔드포인트가 있어야 한다**. 최소안: `GET /api/remote/projects` — `OPEN_ALIVE_TICKET_ROOTS` 하위의 프로젝트와 각 브랜치 목록만 반환.

### 7.3 최소 구현 범위
- 호스트 입력(예: `http://mac.tailnet.ts.net:3141`) + 연결 테스트(`/health`).
- 토큰 입력·보관. QR 페어링은 선택(서버가 `open-alive token --qr` 출력).
...
| `OPEN_ALIVE_REMOTE=1`, 토큰 없음 | 부팅 거부 + 사유 출력 |
| `OPEN_ALIVE_HOST=0.0.0.0`, remote 아님 | 부팅 거부 + 사유 출력 |
...
관전과 조작이 코드상 이미 분리돼 있었다 — `terminal:attach` 는 출력 구독일 뿐이고 타이핑은 `terminal:input` 이다. 그래서 하나의 스위치가 아니라 3단계로 열었다: `OPEN_ALIVE_REMOTE_TERMINAL=off|watch|input|shell`, 기본 `off`, 인식 못 하는 값도 `off`.
