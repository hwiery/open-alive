/**
 * `oa-delegate` — the sub-agent delegation tool the orchestrator Claude calls
 ...
 *
 * A shell wrapper is written to ~/.open-alive/bin/oa-delegate at startup
...
/**
 * The model oa-delegate uses when the caller passes no `--model`:
 * `OA_DELEGATE_MODEL`, then models.json `defaultModel`, then the preset default.
 */
export function resolveDelegateModel(env: NodeJS.ProcessEnv): string {
  return env.OA_DELEGATE_MODEL?.trim() || ACTIVE_DELEGATE_CATALOG.defaultModel || DEFAULT_DELEGATE_MODEL;
}

/** Where oa-delegate appends one JSON line per delegation (server reads by ticketId). */
export const DELEGATION_LOG = join(homedir(), '.open-alive', 'delegations.jsonl');
...
function renderCatalogue(cooldowns: CooldownMap, now: number): string {
  ...
  return [
    ...
    ...rows,
    '',
    'usage: oa-delegate [--model <id|alias|a,b,c>] [--no-fallback] "<prompt>"',
...
export async function runDelegateCli(
  ...
  const cooldowns = deps.cooldowns ?? createCooldownStore(env.OA_DELEGATE_COOLDOWNS || COOLDOWN_PATH);
  ...
  if (!prompt) return { code: 2, stdout: '', stderr: 'oa-delegate: no prompt (pass as arg or stdin)' };
  ...
  const chat =
    ...
    (async (m: string, p: string) => {
      ...
      const timeoutMs = Number(env.OA_DELEGATE_TIMEOUT_MS) || DEFAULT_DELEGATE_TIMEOUT_MS;
  ...
  const chain = buildDelegateChain(requested, {
    noFallback,
    ...(env.OA_DELEGATE_FALLBACKS ? { fallbackOverride: env.OA_DELEGATE_FALLBACKS } : {}),
  });
  ...
  return {
    code: 1,
    stdout: '',
    stderr: `oa-delegate: ${last?.error ?? 'no model available'} ${JSON.stringify({ attempts, ...(skipped.length ? { skippedCooling: skipped } : {}) })}`,
  };
}

/**
 * Record the delegation so the server can attach it to the ticket (which models
 * did what). Keyed by OA_TICKET_ID, set only for orchestrated main runs (not the
...
function logDelegation(o: {
  ...
  const ticketId = o.env.OA_TICKET_ID;
  if (!ticketId) return;
  const record = {
    ticketId,
    model: o.model,
  ...
  const appendLog =
    ...
    ((line: string) => {
      try {
        mkdirSync(join(homedir(), '.open-alive'), { recursive: true });
        appendFileSync(o.env.OA_DELEGATE_LOG ?? DELEGATION_LOG, line + '\n');
      } catch {
...
/**
 * Write the `oa-delegate` wrapper into ~/.open-alive/bin and return its
...
export function ensureDelegateCli(): string | null {
  const binDir = join(homedir(), '.open-alive', 'bin');
  const wrapper = join(binDir, 'oa-delegate');
...
if (
  ...
  // daemon's — so LITELLM_KEY may only exist in ~/.open-alive/.env. Read it
