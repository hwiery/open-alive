/**
 * `open-alive-prompt backfill` — scan / import historical user prompts from
...
async function runDryRun(opts: ResolvedOptions): Promise<void> {
  ...
  if (stats.newPrompts > 0) {
    console.log(
      pc.dim(
        `to import these ${stats.newPrompts.toLocaleString()} prompts: open-alive-prompt backfill --execute`
      )
    );
  }
}
...
async function runExecute(opts: ResolvedOptions): Promise<void> {
  ...
  console.log('');
  console.log(pc.dim('open the open-alive Prompt tab to see the new rows'));
}
