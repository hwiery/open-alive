...
// Section defaults are factories that parse `{}`, not a bare `{}`: zod 4
// returns a bare default as-is without applying the inner field defaults, so
// a fresh install got `agent.max_prompt_bytes === undefined` (Fastify then
// refused `bodyLimit: NaN` and the prompt subsystem never started).
