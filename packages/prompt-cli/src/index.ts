...
program
  ...
  .option(
    '--no-dashboard',
    'skip the dashboard daemon (agent + worker only) — for embedders like open-alive'
  )
...
program
  ...
  .option('--purge', 'also delete ~/.open-alive/prompt/')
...
// Library-only usage: open-alive imports the per-command modules directly
// (e.g. `import { installCmd } from '@open-alive/prompt-cli/dist/commands/install.js'`)
