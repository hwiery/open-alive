...
/**
 * Legacy hook entries from before open-alive absorbed open-alive-prompt
 * (D-048+). The standalone open-alive-prompt agent listened on
...
function isOpenAliveHook(entry: HookEntry): boolean {
  return (
    entry.hooks?.some(
      (h) => typeof h.command === 'string' && h.command.includes('open-alive')
    ) ?? false
  );
}
...
export function installHooks(): { installed: boolean; settingsPath: string; hookScriptPath: string } {
  ...
  const aliveDir = join(home, '.open-alive', 'hooks');
  const settingsPath = join(claudeDir, 'settings.json');

  // 1. Copy hook script to ~/.open-alive/hooks/
  ...
  for (const event of HOOK_EVENTS_TO_REGISTER) {
    ...
    // Drop any legacy open-alive-prompt http hooks that targeted the old
    // standalone agent on :47823; the unified server fans events out
    // internally now, so a single open-alive entry per event suffices.
    const pruned = existing.filter((entry) => !isLegacyThinkPromptHook(entry));
    const alreadyInstalled = pruned.some(isOpenAliveHook);
    ...
    if (settings.hooks) {
      for (const event of Object.keys(settings.hooks)) {
        settings.hooks[event] = (settings.hooks[event] ?? []).filter(
          (entry) => !isOpenAliveHook(entry) && !isLegacyThinkPromptHook(entry)
        );
        if (settings.hooks[event]!.length === 0) {
          delete settings.hooks[event];
        }
