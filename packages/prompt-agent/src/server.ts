...
export function buildAgentServer(deps: AgentDeps = {}): FastifyInstance {
  ...
  // custom `X-Open-Alive-Prompt-Ext` header. Browsers treat that as a
    ...
    try {
      ...
      if (config.agent.coach_mode && (final_score < 65 || hits.some((h) => h.severity >= 3))) {
        const issueLines = hits
          ...
          .join('\n');
        const hint = [
          '[open-alive prompt coaching hint]',
  ...
  //
  // Soft-auth via the `X-Open-Alive-Prompt-Ext` header: the agent binds to 127.0.0.1
  ...
  // ---------------------------------------------------------------------
  // JSON API for embedders (e.g. open-alive's React UI).
  //
