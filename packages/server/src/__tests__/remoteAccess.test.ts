...
describe('parseTokens', () => {
  it('reads the single-token variable', () => {
    expect(parseTokens({ OPEN_ALIVE_TOKEN: LONG })).toEqual([{ label: 'default', value: LONG }]);
  });
  it('reads labelled entries and trims blanks', () => {
    expect(parseTokens({ OPEN_ALIVE_TOKENS: ` phone:${LONG} , ipad:${OTHER} , ` })).toEqual([
      { label: 'phone', value: LONG },
      { label: 'ipad', value: OTHER },
    ]);
  });
  it('accepts a bare value in the list form', () => {
    expect(parseTokens({ OPEN_ALIVE_TOKENS: LONG })).toEqual([{ label: 'token1', value: LONG }]);
  });
});
...
describe('bearerFromHeaders', () => {
  ...
  it('reads the websocket subprotocol form', () => {
    expect(bearerFromHeaders({ 'sec-websocket-protocol': `open-alive.token.${LONG}` })).toBe(LONG);
  });
...
describe('loadRemoteAccessConfig', () => {
  ...
  it('refuses a non-loopback host without remote mode', () => {
    const r = loadRemoteAccessConfig({ OPEN_ALIVE_HOST: '0.0.0.0' });
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.errors.join(' ')).toContain('OPEN_ALIVE_REMOTE');
  });
  it('refuses remote mode without a token', () => {
    const r = loadRemoteAccessConfig({ OPEN_ALIVE_REMOTE: '1', OPEN_ALIVE_TICKET_ROOTS: '/work' });
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.errors.join(' ')).toContain('OPEN_ALIVE_TOKEN');
  });
  it('refuses a short token', () => {
    const r = loadRemoteAccessConfig({ OPEN_ALIVE_REMOTE: '1', OPEN_ALIVE_TOKEN: 'short', OPEN_ALIVE_TICKET_ROOTS: '/work' });
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.errors.join(' ')).toMatch(/24|characters/);
  });
  it('refuses remote mode without a ticket-root allowlist', () => {
    const r = loadRemoteAccessConfig({ OPEN_ALIVE_REMOTE: '1', OPEN_ALIVE_TOKEN: LONG });
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.errors.join(' ')).toContain('OPEN_ALIVE_TICKET_ROOTS');
  });
  it('accepts a complete remote configuration and binds all interfaces by default', () => {
    const r = loadRemoteAccessConfig({
      OPEN_ALIVE_REMOTE: '1',
      OPEN_ALIVE_TOKEN: LONG,
      OPEN_ALIVE_TICKET_ROOTS: '/work:/src',
      OPEN_ALIVE_REMOTE_SSH_HOSTS: 'build.local',
    });
  ...
  it('keeps an explicit host in remote mode', () => {
    const r = loadRemoteAccessConfig({
      OPEN_ALIVE_REMOTE: '1', OPEN_ALIVE_HOST: '100.64.0.1',
      OPEN_ALIVE_TOKEN: LONG, OPEN_ALIVE_TICKET_ROOTS: '/work',
    });
