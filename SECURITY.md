...
If you discover a security vulnerability in open-alive, please report it responsibly.
...
open-alive runs a local server that can start autonomous agents, so the
...
Remote access is off by default and is enabled with `OPEN_ALIVE_REMOTE=1`.
Turning it on changes the model rather than widening it:

- The server refuses to start without a device token (`OPEN_ALIVE_TOKENS`) and
  a cwd allowlist (`OPEN_ALIVE_TICKET_ROOTS`)
...
- A device connection to the WebSocket may read the stream. Terminal access is
  a separate, graduated opt-in (`OPEN_ALIVE_REMOTE_TERMINAL`, default `off`):
...
- Tickets created by a device must fall inside the cwd allowlist, and SSH-located
  tickets require the host to be named in `OPEN_ALIVE_REMOTE_SSH_HOSTS`
...
- Local components (the hook script, the CLI, the served dashboard) use a
  separate full-access token generated into `~/.open-alive/.env` with mode
...
- A device token is a bearer credential: anyone holding it can create tickets in
  ...
  `open-alive token revoke <label>`
- In local-only mode the WebSocket endpoint is open to any local process

## Trademark Notice

"Claude" and "Claude Code" are trademarks of Anthropic, PBC. open-alive is an independent, community-driven project that works with Claude Code; it is not affiliated with, sponsored by, or endorsed by Anthropic. References to Claude Code describe compatibility only.
