...
describe('notificationContent', () => {
  ...
  describe('folderLabel', () => {
    it('keeps only the last folder of the path', () => {
      ...
      expect(folderLabel('/Users/jane/a/b/c/d/open-alive')).toBe('open-alive');
  ...
  describe('buildAlertContent', () => {
    it('leads with the project name and the stage, never a session id', () => {
      const content = buildAlertContent('waiting', {
        projectName: 'open-alive',
        cwd: '/Users/jane/Documents/open-alive',
        tool: 'Bash',
        lastPrompt: 'move the toast to the bottom left',
      });
      expect(content.title).toBe('open-alive · Needs permission');
      expect(content.body).toContain('Folder: open-alive');
