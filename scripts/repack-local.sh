...
#      previous global install of `open-alive`)
...
# `npm i -g open-alive`. Earlier we did `npm i -g <dir>` directly, but npm
...
open-alive stop > /dev/null 2>&1 || true
...
sleep 1
open-alive start --no-open

INSTALLED_VERSION=$(open-alive --help 2>/dev/null | head -1 || echo "unknown")
echo ""
echo "Done. Global open-alive now reflects your local changes."
echo "  Dashboard: http://localhost:${OPEN_ALIVE_PORT:-3141}"
