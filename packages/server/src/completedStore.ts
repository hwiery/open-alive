...
import type { CompletedSession, TokenUsage } from '@open-alive/core';
 ...
 *
 * Stored oldest→newest as a JSON array in ~/.open-alive/completed-sessions.json.
 * Capped at MAX_ENTRIES so the file can't grow without bound on a long-lived host.
 */

const ARCHIVE_FILE = join(homedir(), '.open-alive', 'completed-sessions.json');
...
async function flush(): Promise<void> {
  ...
  await mkdir(join(homedir(), '.open-alive'), { recursive: true });
