/**
 * Reads the `oa-delegate` delegation log and returns the records for a ticket.
 *
 * oa-delegate appends one JSON line per delegation (tagged with OA_TICKET_ID);
 * the runner attaches the matching records to the ticket on settle so the UI can
 * show which sub-agent models did what.
 */
import { readFileSync } from 'node:fs';
import type { TicketDelegation } from '@open-alive/core';
