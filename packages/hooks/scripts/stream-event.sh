#!/bin/bash
# open-alive hook: streams Claude Code events to the local server.
...
AUTH=()
PORT="${OPEN_ALIVE_PORT:-}"
ENV_FILE="${HOME}/.open-alive/.env"
if [ -r "$ENV_FILE" ]; then
  # A port chosen in `open-alive setup` lives in the env file, not in Claude Code's env.
  if [ -z "$PORT" ]; then
    PORT=$(sed -n 's/^[[:space:]]*\(export[[:space:]]\{1,\}\)\{0,1\}OPEN_ALIVE_PORT=//p' "$ENV_FILE" | tail -n 1 | sed "s/^['\"]//; s/['\"]$//")
  fi
  TOKEN=$(sed -n 's/^[[:space:]]*\(export[[:space:]]\{1,\}\)\{0,1\}OPEN_ALIVE_LOCAL_TOKEN=//p' "$ENV_FILE" | tail -n 1 | sed "s/^['\"]//; s/['\"]$//")
  if [ -n "$TOKEN" ]; then
    AUTH=(-H "Authorization: Bearer ${TOKEN}")
  fi
fi

case "$PORT" in (""|*[!0-9]*) PORT=3141 ;; esac

curl -s -X POST "http://localhost:${PORT}/api/event" \
