...
describe('readEnvValue', () => {
  it('reads a key, tolerating export and quotes', () => {
    expect(readEnvValue('export OPEN_ALIVE_LOCAL_TOKEN="abc"\n', 'OPEN_ALIVE_LOCAL_TOKEN')).toBe('abc');
  });
  it('returns undefined for a missing key', () => {
    expect(readEnvValue('OTHER=1\n', 'OPEN_ALIVE_LOCAL_TOKEN')).toBeUndefined();
  });
  it('ignores a commented-out line', () => {
    expect(readEnvValue('#OPEN_ALIVE_TOKENS=x\n', 'OPEN_ALIVE_TOKENS')).toBeUndefined();
  });
});
...
describe('device token list', () => {
  it('adds the first token', () => {
    ...
    expect(next).toContain(`OPEN_ALIVE_TOKENS=phone:${A}`);
