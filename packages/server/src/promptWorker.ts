/**
 * Thin wrapper around the absorbed @open-alive/prompt-worker library. The
 * worker now runs in-process inside the open-alive server (no fork,
 * no pidfile). This module exists only so index.ts has one symbol to
 * call and one shutdown handle to invoke.
 */
import { startWorkerLoop as start } from '@open-alive/prompt-worker';
