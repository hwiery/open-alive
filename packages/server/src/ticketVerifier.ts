 ...
 */
import type { Ticket, TicketVerification, TicketLocation } from '@open-alive/core';
...
export function buildVerificationPrompt(goal: string, mainResult: string | null, orchestrated = false): string {
  const orchestrationNote = orchestrated
    ? [
        ...
        'delegated subtasks to sub-agents by running the `oa-delegate` tool (on PATH), which',
        ...
        'file changes or reject solely because `oa-delegate` output was network-based.',
      ]
    : [];
  return [
