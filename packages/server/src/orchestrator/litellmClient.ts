 ...
 * and chat completion for the `oa-delegate` sub-agent tool. The API key lives in
 * server env only and is never sent to the browser. `fetch` is injectable so the
 * client is testable without network.
 */
import { randomUUID } from 'node:crypto';

/** Used when `LITELLM_BASE_URL` is unset: a LiteLLM proxy on its default local port. */
...
export function createLitellmClient(config: LitellmConfig, deps: { fetch?: FetchFn } = {}): LitellmClient {
  ...
  const sessionId = config.sessionId?.trim() || `open-alive-${randomUUID()}`;
