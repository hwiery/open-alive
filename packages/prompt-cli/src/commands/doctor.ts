...
import { getMeta, getPaths, loadConfig, openDb } from '@open-alive/prompt-core';
...
export async function doctorCmd(): Promise<void> {
  ...
  console.log(pc.bold('open-alive prompt Doctor'));
  console.log('─────────────────');
  ...
  if (hooksPresent(paths.claudeSettings)) {
    ...
    console.log(pc.yellow('⚠') + ' hooks NOT present — run `open-alive-prompt install`');
  } else {
